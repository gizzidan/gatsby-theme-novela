import prism from "./prism";

export default {
  prism,

  primary: "#131516",
  secondary: "#6a6c6f",
  onAccent: "#fff",
  grey: "#6a6c6f",
  lightGrey: "#e1e1e1",
  darkGrey: "#46454d",
  navbar: "#fff",
  navbarSecond: "#f3c5eb",
  navBackground: "#131516",
  navBorder: "transparent",
  background: "#fff",
  accent: "#4524EA",
  sponsored: "#131516",
  tagBackground: "#e1dfe6",
  featureBackground: 'linear-gradient(to bottom, rgba(54, 28, 182, .8), rgba(168, 54, 92, 0.8))',
  outline: "#000",
  hover: "rgba(0, 0, 0, 0.07)",
  gradient: "linear-gradient(to bottom, #fff, #f6eafd)",
  navGradient: "linear-gradient(to right, rgba(255,255,255,0), #fff 60% )",
  OGgradient: "linear-gradient(180deg, rgba(217, 219, 224, 0) 0%, #D9DBE0 100%)",
  articleText: "#000",
  track: "rgba(8, 8, 11, 0.3)",
  progress: "#000",
  card: "linear-gradient(to right, #fff, #fff)",
  cardMain: "#fff",
  blockquote: '#eeedf1',
  error: "#EE565B",
  success: "#46B17B",
  errorBackground: "rgba(238, 86, 91, 0.1)",
  horizontalRule: "rgba(8, 8, 11, 0.15)",
  inputBackground: "rgba(0, 0, 0, 0.03)",
  modes: {
    dark: {
      grey: "#d2d2d3",
      lightGrey: "#393c43",
      darkGrey: "#d3d2db",
      navbar: "#fff",
      navBackground: "#171922",
      featureBackground: 'linear-gradient(to bottom, rgba(138, 134, 161, .6), rgba(15, 15, 19, .95))',
      navBorder: "#393c43",
      primary: "rgba(255, 255, 255, 0.98)",
      secondary: "#000",
      onAccent: "#333",
      accent: "#ea98dc",
      tagBackground: "#d3d1db",
      outline: "#13161d",
      background: "#171922",
      hover: "rgba(255, 255, 255, 0.07)",
      gradient: "linear-gradient(to bottom, #171922, #403d4e)",
      navGradient: "linear-gradient(to right, rgba(21,32,43,0), #171922 60% )",
      OGgradient:
        "linear-gradient(180deg, #171922 0%, rgba(66, 81, 98, 0.36) 100%)",
      articleText: "rgba(255, 255, 255, 0.90)",
      track: "rgba(255, 255, 255, 0.3)",
      progress: "#fff",
      card: "#171922",
      cardMain: "#171922",
      blockquote: '#403d4d',
      error: "#EE565B",
      success: "#46B17B",
      errorBackground: "rgba(238, 86, 91, 0.1)",
      horizontalRule: "rgba(255, 255, 255, 0.15)",
      inputBackground: "rgba(0, 0, 0, 0.03)",
    },
  },
};
